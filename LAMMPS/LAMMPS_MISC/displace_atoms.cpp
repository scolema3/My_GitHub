...
enum{MOVE,RAMP,RANDOM,ROTATE,STRETCH};
...
void DisplaceAtoms::command(int narg, char **arg)
{
  ...
  else if (strcmp(arg[1],"stretch") == 0) style = STRETCH;  
  ...
  else if (style == STRETCH) options(narg-4,&arg[4]);  
  ...
  // stretch atoms equally in a box direction
  ...
  if (style == STRETCH) {
    ...
    else error->all(FLERR,"Illegal displace_atoms stretch command");

    double expand = force->numeric(FLERR,arg[3]);
    double **x = atom->x;
    int *mask = atom->mask;
    int nlocal = atom->nlocal;
