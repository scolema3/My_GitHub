...
FixXRDvtk::FixXRDvtk(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg)
{
  if (narg < 7) error->all(FLERR,"Illegal fix saed/xrd command");
  ...
  if (nvalues != 1) error->all(FLERR,"Illegal fix saed/xrd command");
  ...
  while (iarg < narg) {
    if (strncmp(arg[iarg],"c_",2) == 0 ) {
      ...
      if (ptr) error->all(FLERR,"Illegal fix saed/xrd command");
      ...
      if (icompute < 0)
        error->all(FLERR,"Compute ID for fix saed/xrd does not exist");
      ...
      if (strcmp(compute_xrd->style,"xrd") != 0)
        error->all(FLERR,"Fix saed/xrd has invalid compute assigned");
  ...
  if (nevery <= 0 || nrepeat <= 0 || nfreq <= 0)
    error->all(FLERR,"Illegal fix saed/xrd command");
  ...
  if (ave == WINDOW)
    memory->create(vector_list,nwindow,nvalues,"saed/xrd:vector_list");

  memory->create(vector,nrows,"saed/xrd:vector");
  memory->create(vector_total,nrows,"saed/xrd:vector_total");
...
void FixXRDvtk::invoke_vector(bigint ntimestep)
{
  ...
  if (fp && me == 0) {
    if (nOutput > 0) {
      ...
      if (fp == NULL) {
        char str[128];
        sprintf(str,"Cannot open fix saed/xrd file %s",nName);
        error->one(FLERR,str);
      }
    }
...
void FixXRDvtk::options(int narg, char **arg)
{
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg],"file") == 0) {
      if (iarg+2 > narg) error->all(FLERR,"Illegal fix saed/xrd command");
      if (me == 0) {
      ...
      else error->all(FLERR,"Illegal fix saed/xrd command");
      if (ave == WINDOW) {
        if (iarg+3 > narg) error->all(FLERR,"Illegal fix saed/xrd command");
        nwindow = force->inumeric(FLERR,arg[iarg+2]);
        if (nwindow <= 0) error->all(FLERR,"Illegal fix saed/xrd command");
      }
      iarg += 2;
      if (ave == WINDOW) iarg++;
...
{
  if (ntimestep > nvalid) error->all(FLERR,"Fix saed/xrd missed timestep");
}
