...
ComputeStyle(saedmic,ComputeSAEDMIC)

#else

#ifndef LMP_COMPUTE_SAEDMIC_H
#define LMP_COMPUTE_SAEDMIC_H
...
class ComputeSAEDMIC : public Compute {
 public:
  ComputeSAEDMIC(class LAMMPS *, int, char **);
  ~ComputeSAEDMIC();
  void    init();
  void    compute_vector();
  double  memory_usage();
